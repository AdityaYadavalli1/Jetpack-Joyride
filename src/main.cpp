...
void tick_elements() {
    ...
    // if( tickcounter == magnetdisappear)
    // {
    //       tickcounter=0;
    ...
    for (int i=0; i<4; i++)
    {
      if(detect_collision(ball1.bounding_box(),doublecoins[i].bounding_box()))
      {
        doublecoins[i].set_position(-100,-100);
      }
    }
    if(under_magnet_influence(ball1.bounding_box(),magnet.bounding_box()))
    {
